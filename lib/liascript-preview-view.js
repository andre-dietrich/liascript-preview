...
class LiascriptPreviewView  {
  ...
  constructor (uri, editorId, base_url) {
    ...
    this.iframe.src = base_url+"?file://"+path;
    ...
    */
    ...
    let jit = function () {
      if (atom.config.get('liascript-preview.experimental_jit'))
        iframe_.contentWindow.postMessage({cmd: "jit", param: editor_.getText()});
    ...
    */
    ...
    atom.views.getView(this.editor).addEventListener("dblclick", (e) => {
      try {
        ...
        iframe_.contentWindow.postMessage({cmd: "goto", param: line});
      } catch (e) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'liascript-preview:open-in-browser': () => {
        //if(this.editor != null){
          console.log("open", this.webview.iframe.src);
          //shell.openExternal(this.webview.src);
          window.open(this.webview.iframe.src, '_blank')
          //shell.openExternal(this.webview.src);
        //}
      },
    }));
  ...
  reload() {
    if (this.iframe.src.endsWith("index.html")) {
      this.webview.goBack();
    } else {
      //this.iframe.contentWindow.location.reload(true);//src = this.iframe.contentWindow.location.href;
      this.iframe.contentWindow.postMessage({cmd: "reload", param: null});
    }
  }
